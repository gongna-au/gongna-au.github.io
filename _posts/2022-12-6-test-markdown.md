...
每个P都有自己的本地Goroutine队列，这提高了缓存局部性并减少了锁竞争。
全局锁被移除，取而代之的是更细粒度的锁，如P的本地锁。
P的数量通常与CPU核心数相同，这为Go的调度提供了一个自然的并行度，确保了在多核机器上的可伸缩性。
因此，GMP模型不仅解决了全局锁的问题，还为Go提供了一个更高效、更可伸缩的并发模型。
