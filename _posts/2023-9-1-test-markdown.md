...
2. **定义度量**：在你的代码中定义需要的度量。比如，你可能需要创建一个计数器来跟踪接收到的请求总数：
    ...
    上面的代码创建了一个计数器`totalRequests`，它将按照HTTP方法和端点分类统计请求总数。
...
所以，虽然添加一个新的路由（如"/metrics"）是暴露度量数据的一部分，但真正的数据收集应该在你的业务逻辑中进行。我希望这能回答你的问题，如果你还有其他问题或需要进一步的澄清，请随时告诉我。
...
这个解决方案的关键在于找到一个适合暴露 Prometheus 度量的地方，而且这个地方需要能够处理 HTTP 请求。我希望这个答案能解决你的问题，如果你还有其他问题，欢迎继续提问。
